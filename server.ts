...
async function startServer() {
  ...
  // Serve image files from the project root directly if they exist
  app.get(/^\/(.*\.png|.*\.jpg|.*\.jpeg|.*\.gif|.*\.svg|.*\.webp)$/, (req, res, next) => {
    const filePath = path.join(process.cwd(), req.path);
    res.sendFile(filePath, (err) => {
      if (err) {
        next(); // If not found in root, let Vite/Static handle it
      }
    });
  });
